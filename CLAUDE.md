...
| `persistZone` / `undoOnce` | 필기 되돌리기. 영역(zone) HTML을 통째로 갈아끼우는 구조라 바뀌기 직전 HTML만 `UNDO`에 쌓음. 메모 타이핑은 `soft` 표시로 한 덩어리로 묶음. ⌘/Ctrl+Z, 화면 위 「↶ 되돌리기」 |
...
- **한 항목에 대본이 여러 개면 대본 단위로 쪼개서 각각을 별개 지문으로 봄**
  (`mockSources`). 교재 한 세트(t2-*.json)가 16문항/대본 8종이라 통째로
  뽑으면 구성이 어긋남. 연습 탭(리스닝 목록)은 교재 세트를 그대로 유지함
